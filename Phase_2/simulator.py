...
def show_registers():
    ...
    for reg in Reg.keys():
        s=reg + " (x" + str(i) + ") "
        colon=":"
        f=s.center(8)+colon.center(2) +str(Reg[reg])
        print(f.center(20),end="        ")
        j=j+1
        if(j%6==0):
            print()
        i = i + 1
...
def show_memory():
    ...
    for i in range(len(data['.word'])):
        f=hex((base_address + 4 * i)).ljust(11) + ": " + str(data['.word'][i])
        print(f, end="        ")
        j=j+1
        if(j%6==0):
            print()
...
fileopen = open("programs/bubble_sort.s", "r+")
...
def process_simulate():
    ...
    for ins in data_and_text['text']:
        ...
        print(str(index) + ":" + str(text_memory[index]).center(35))
